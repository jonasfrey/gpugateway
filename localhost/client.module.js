...
import { O_gpu_gateway, O_gpu_gateway_webgpu, O_gpu_gateway_webgpu_dataitem, O_gpu_texture, O_gpu_texture_collection, O_gpu_texture_collection_item, O_shader_error, O_shader_info, O_texture } from "./classes.module.js";
...
let f_o_gpu_gateway_webgpu = async function(
    ...
    const s_presentationFormat = o_gpu.getPreferredCanvasFormat();
    ...
    let a_o_gpu_gateway_webgpu_dataitem = Object.keys(o_data).map((s_prop, n_idx)=>{
        let v = o_data[s_prop];
        if(!typeof v == 'number'){
            console.error(`s_prop: ${s_prop},s_type: ${typeof v} invalid, only typeof number is allowed at the moment`)
            return false;
        }
        let n_bytes = (typeof v == 'number') ? 4 : 0;
        return new O_gpu_gateway_webgpu_dataitem(
            null,
            s_prop,
            n_idx,
            n_bytes, 
            o_device.createBuffer({
                size: n_bytes, // sizeof(f32) = 4 bytes
    ...
    const bindGroupLayout = o_device.createBindGroupLayout({
        entries: [
            ...a_o_gpu_gateway_webgpu_dataitem.map(o=>{
                return {
                    binding: o.n_idx,
    ...
    const bindGroup = o_device.createBindGroup({
    ...
    const pipelineLayout = o_device.createPipelineLayout({
        bindGroupLayouts: [bindGroupLayout]
    });
    const o_pipeline = o_device.createRenderPipeline({
      layout: pipelineLayout,
      vertex: {
        module: o_device.createShaderModule({
    ...
    let o_gpu_gateway_webgpu = new O_gpu_gateway_webgpu(
        o_gpu,
        o_adapter,
        o_device,
        o_canvas,
    ...
    a_o_gpu_gateway_webgpu_dataitem.forEach(o=>o.o_gpu_gateway_webgpu = o_gpu_gateway_webgpu)
...
let f_s_data_accessor_code = function(o_data){
    return Object.keys(o_data).map(
        (s_prop, n_idx)=>{
            return `
                @group(0) @binding(${n_idx}) var<uniform> ${s_prop} : f32;
            `
        }
    )
}
let f_update_data_in_o_gpu_gateway_webgpu = function(
    ...
    for(let s_prop in o_data){
        ...
        o_gpu_gateway_webgpu.o_device.queue.writeBuffer(
            o_gpu_gateway_webgpu_dataitem.o_buffer,
            0,//n_idx_offset
            new Float32Array([v])
        );
    }
